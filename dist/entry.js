...
function taskFactory(task, db) {
  switch (task.name) {
    case 'build-email-pdfs':
      return new _BuildEmailPdfsTask2.default(db, task.props, config);
    default:
      (0, _logHelper.log)('status', 'Could not fine task named ' + task.name + '.');
...
dbHelper.connectToDatabase().then(function (db) {
